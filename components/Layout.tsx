...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, user, onLogout }) => {
  ...
  // Primary mobile actions (Home, Messages, Progress, etc.)
  ...
  return (
    ...
    <div className="h-screen w-full flex flex-col md:flex-row bg-slate-50 dark:bg-slate-950 overflow-hidden selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <div className="flex-1 min-w-0 flex flex-col h-full relative">
        ...
        <header className="hidden md:flex items-center justify-between px-10 h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b dark:border-slate-800 shrink-0 z-30">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="text-right">
              <p className="text-xs font-extrabold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest leading-none">{user.institution}</p>
              <div className="flex items-center justify-end gap-1.5 mt-1.5">
                <span className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase">Batch {user.batch}</span>
                <div className="w-1 h-1 bg-slate-300 dark:bg-slate-700 rounded-full" />
                <span className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase">Term {user.term}</span>
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-800">
        ...
        <nav className="md:hidden h-16 bg-white dark:bg-slate-900 border-t dark:border-slate-800 flex items-center justify-around px-2 shrink-0 z-50">
          {mobileNavItems.map((item) => (
            ...
            >
              {/* Fixed: Cast to React.ReactElement<any> to resolve the 'size' property error on line 184 */}
