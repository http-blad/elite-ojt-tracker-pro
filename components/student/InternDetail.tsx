...
import { Intern, TrainingPlanModule } from '@/types';
...
const InternDetails: React.FC<InternDetailsProps> = ({ intern, onBack, onUpdate, onCreateAccount, isAccountCreated }) => {
  ...
  const handleAnalyzeProgress = async () => {
    if (intern.logs.length === 0) {
      alert("No logs available to analyze.");
      return;
    }
    setIsAnalyzing(true);
    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button className="px-5 py-2 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 dark:shadow-none">
            Edit Profile
          </button>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-8 rounded-3xl shadow-xl shadow-slate-200 dark:shadow-none text-white relative overflow-hidden">
            ...
            <p className="text-slate-300 text-sm mb-6 leading-relaxed">
              Analyze performance and generate roadmaps tailored to the curriculum.
            </p>
          ...
          {analysisResult && (
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 p-6 rounded-3xl text-emerald-900 dark:text-emerald-100">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="font-bold text-sm uppercase tracking-wider">AI Progress Summary</h4>
              </div>
            ...
            <div className="p-4 md:p-8">
              {intern.trainingPlan ? (
                ...
                <div className="py-20 flex flex-col items-center justify-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 font-medium text-center max-w-sm">No roadmap generated yet.</p>
                </div>
              )}
            </div>
          </div>
                  ...
                  {intern.logs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-8 py-5">
                        ...
                        {log.proofImage && (
                          <div className="mt-2 inline-flex items-center gap-2 px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-[10px] font-bold text-slate-500">
                             Proof Attached
                          </div>
                        )}
                      </td>
